...
export declare const combineStackGroups: (displayedData: ChartData, items: ReadonlyArray<MaybeStackedGraphicalItem>, stackOffsetType: StackOffsetType) => Record<StackId, StackGroup>;
/**
 * Stack groups are groups of graphical items that stack on each other.
 * Stack is a function of axis type (X, Y), axis ID, and stack ID.
 * Graphical items that do not have a stack ID are not going to be present in stack groups.
 */
export declare const selectStackGroups: (state: RechartsRootState, axisType: XorYorZType, axisId: AxisId, isPanorama: boolean) => Record<StackId, StackGroup> | undefined;
export declare const combineDomainOfStackGroups: (stackGroups: Record<StackId, StackGroup> | undefined, { dataStartIndex, dataEndIndex }: ChartDataState, axisType: XorYorZType) => NumberDomain | undefined;
export declare const selectDomainOfStackGroups: ((state: import("redux").EmptyObject & {
    ...
    resultFunc: (resultFuncArgs_0: Record<StackId, StackGroup>, resultFuncArgs_1: ChartDataState, resultFuncArgs_2: any) => NumberDomain;
    memoizedResultFunc: ((resultFuncArgs_0: Record<StackId, StackGroup>, resultFuncArgs_1: ChartDataState, resultFuncArgs_2: any) => NumberDomain) & {
        clearCache: () => void;
        resultsCount: () => number;
        resetResultsCount: () => void;
    };
    lastResult: () => NumberDomain;
    dependencies: [(state: RechartsRootState, axisType: XorYorZType, axisId: AxisId, isPanorama: boolean) => Record<StackId, StackGroup> | undefined, (state: RechartsRootState) => ChartDataState, <T>(_state: RechartsRootState, axisType: T) => T];
